...
var keys = [];
var walkingInGrassAudio = new Audio();
walkingInGrassAudio.src = "/walking-on-grass.m4a";
walkingInGrassAudio.volume = 0.233;
var walkkingInGrassAudio = new Audio();
walkkingInGrassAudio.src = "/walking-on-grass.m4a";
var walkkkingInGrassAudio = new Audio();
walkkkingInGrassAudio.src = "/walking-on-grass.m4a";
var audioTimers = {
  walkingInGrassAudio: new Date().getTime(),
  walkkingInGrassAudio: new Date().getTime(),
  walkkkingInGrassAudio: new Date().getTime()
};
    ...
    case 'WALK':
      return {
        ...
        model: {
          ...state.model,
          walking: true,
          walkingInGrass: state.level == "meadow"
        }
      }
    case 'STRAFE':
      return {
  ...
  return (
    ...
    <div className="App" style={{ background: state.model.scene ? 'none' : 'url(/dandelion.gif)', backgroundRepeat: 'no-repeat', backgroundSize: 'cover' }}>
      ...
      <CanvasContainer 
        state={state}
        keys={keys}
        walkingInGrassAudio={walkingInGrassAudio}
        dispatch={dispatch} />

      <h1 id="stats"></h1>

      <AudioController start={start} play={{
        genericMeadowMusic: true,
        walkingInGrass: state.model.walkingInGrass
      }} />
    </div>
