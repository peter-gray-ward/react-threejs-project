...
function ModelViewer(props) {
    ...
    useEffect(() => {
        ...
        const intervalId = setInterval(() => {
            ...
            for (let plant of props.state.plants) {
                if (plant.type === "tree") {
                    if (plant.object.position.distanceTo(camera.position) < 10) {
                        closeThings.push({
                            type: 'tree',
                            index: plantIndex
                        });
                    ...
                    props.state.flowerRefCurrent.getMatrixAt(plant.index, matrix);
                    const pos = new Vector3().setFromMatrixPosition(matrix);
                    if (pos.distanceTo(camera.position) < 10) {
                        closeThings.push({
                            type: 'flower',
                            index: plantIndex
                        });
            ...
            props.dispatch({ type: "ADD_CLOSE_THINGS", closeThings });


            var uuidNew = document.getElementById("close-children");
            if (uuidNew) uuidNew.remove();
